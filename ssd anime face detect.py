...
def resizeCrop(img,ymin,ymax,xmin,xmax):
    # ymin -= int(0.08 * (ymax - ymin + 1))
    # min -= int(0.08 * (xmax - xmin + 1))
    # ymax += int(0.08 * (ymax - ymin + 1))
    # xmax += int(0.08 * (xmax - xmin + 1))
    im_height, im_width, _ = img.shape
    if ymin < 0: ymin = 0
    if xmin < 0: xmin = 0
    if xmax > im_width: xmax = im_width
    if ymax > im_height: ymax = im_height

    yCenter = int((ymax - ymin) / 2)
    xCenter = int((xmax - xmin) / 2)
    newavg = int((yCenter + xCenter) / 2)
    ymin = ymin + yCenter - newavg
    ymax = ymin + yCenter + newavg
    xmin = xmin + xCenter - newavg
    xmax = xmin + xCenter + newavg
    if ymin < 0:
        ymax -= ymin
        ymin = 0
    elif ymax > im_height:
...
def ssd_anime_face_detect(image_path, model_path):
    ...
    for f in listdir(image_path):
        imgOrig = cv2.imread(image_path+f, cv2.IMREAD_COLOR) if type(image_path)==str else image_path
        ...
        for k in range(dets.shape[0]):
            ...
            ymin,ymax,xmin,xmax = resizeCrop(imgOrig,ymin,ymax,xmin,xmax)
            score = dets[k, 4]
            ...
            if score >=.5:
                image = imgOrig[ymin:ymax,xmin:xmax]
                image = cv2.resize(image,(512,512),interpolation=cv2.INTER_AREA)
                cv2.imwrite('faces/'+str(score)+'.jpg', image)
                count+=1



if __name__ =='__main__':
    ssd_anime_face_detect('data/', 'ssd_anime_face_detect.pth')
